...
更完整的财务与行业背景见 [A 股量化研究所需的财务、行业与估值基础](Lesson-0/background-financial-fundamentals.md)。其中直接解释了各行业 PE/PB/PS 的差异、贵便宜的粗略观察区间、周期 PE 陷阱、政策敏感行业以及如何把财务知识转成量化因子。

宏观数据、政策传导、公司消息、price-in 和趋势状态的系统框架见 [A 股量化研究所需的宏观、政策、消息与趋势背景](Lesson-0/background-macro-policy-news-regime.md)。其中把“政策利好行业”“消息是否已计价”和“趋势是否延续”等口语问题改写成点时数据、公司暴露、事件研究与条件概率。

股票与居民存款、房价、利率、信用、汇率、商品、港口货流、生猪期货、海外市场和行业经营数据之间的关系，见 [A 股量化研究中的跨资产、实体经济与另类指标](Lesson-0/background-cross-asset-indicators.md)。其中区分了机械关系、状态依赖的经济关系和伪相关，并给出了从经济机制到样本外检验的完整路线。
...
行情软件中常见的成交价、换手率、量比、五档盘口、资金流、技术指标和“筹码分布”并不是同一可信层级。详细的计算方法、可解释范围与科学性检验见 [A 股行情、成交、盘口与筹码指标基础](Lesson-0/background-market-indicators.md)。其中尤其解释了为什么公开行情无法直接恢复投资者真实持仓成本，以及如何区分真实成交量价格分布和软件商的筹码递推模型。
...
双均线的数学含义、趋势状态与金叉事件的区别、参数选择、震荡失效、ATR 缓冲和评价方法，见 [技术指标背景：双均线趋势策略](Lesson-0/background-market-indicators.md#双均线趋势策略)。
